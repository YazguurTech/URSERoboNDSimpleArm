...
std::vector<float> clamp_at_boundaries(float requested_j1, float requested_j2)
{
	...
	// Get min and max joint parameters, and assigning them to their repective variables
	float min_j1, max_j1, min_j2, max_j2;
	// Assign a new node handles since we have no access to the main one
	ros::NodeHandle n2;
	// Get node name
	...
	// Store clamped joint angles is a clamped_data vector
